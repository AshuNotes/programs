...
def generate_world():
    ...
    elements = ['W', 'G', 'P', 'P']
    pos = random.sample([(i, j) for i in range(grid_size) for j in range(grid_size) if (i, j) != (0, 0)], len(elements))
   
    for (x, y), elem in zip(pos, elements):
        world[x][y] = elem
...
if __name__ == "__main__":
    ...
    print("You cna move up/down/left/right only")
    while True:
        display_state(x, y)
